defmodule WishlistWeb.HcLeagueItemController do
  ...
  def build(conn, %{"ids" => ids}) do
    if is_list(ids) do
      items = Dashboard.list_build_items(HcLeagueItem, ids)
  ...
  def index(conn, _params) do
    items = Dashboard.list_all_items(HcLeagueItem)
    render(conn, "index.json", items: items)
  end

  def find_by_name(conn, %{"name" => name}) do
    items = Dashboard.list_by_name(HcLeagueItem, name)
    render(conn, "index.json", items: items)
  end

  def list_names(conn, _params) do
    items = Dashboard.list_all_names(HcLeagueItem)
    render(conn, "names.json", items: items)
  end

  def pob(conn, %{"url" => url}) do
    items = Dashboard.list_pob_items(HcLeagueItem, url)
    render(conn, "index.json", items: items)
  end
end
